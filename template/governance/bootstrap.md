...
3. **对齐版本/迁移**：跑 `substrate-migrate`（dry-run，不加 --apply）——若 `instance.version < engine.version` 它会列出 pending 迁移。
   ...
   - ★ **本机没有引擎仓库**（大多数 agent 都这样——引擎只在 `migration_leader` 那台）：migrate 会**干净跳过**（提示"本机无引擎，正常"），**不是报错**。原因：迁移脚本只在引擎里、不 vendor 进实例。**升级流程 = leader 迁好实例 → push → 其它机器 pull 实例 + `sync --apply` 重对齐**——其它机器永远跟着实例走，不自己迁。
   - 若实例配了 `governance/ENGINE_SOURCE_URL`，migrate 还会 best-effort 看一眼远程引擎版本，有新版就**提醒**你去 leader 升级（纯提醒，不自动升）。
