...
## 四种入库去向（admission outcome）

对**一条新内容**的准入判定结果（页/条目级）：
...
>
> ⚠️ 别和 zone 级的 `disposition` 字段混淆：那是 `zones.md` 里**整个 zone** 的存储取向，只 `{canonical, reference}` 两值（见 `schemas/zone.schema.yaml`）。此处四种是**单条内容**的去向，层级与取值集都不同。
