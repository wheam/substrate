...
5. **写页**：
   ...
   - YAML frontmatter 必备（与本区 README + doctor 对齐）：
     ...
     > `summary` 可选但**推荐**：常驻上下文（`substrate-runtime-context`）只把每页的 `summary` 列进「记忆目录」、正文不进，agent 需要细节时才现读本页——所以 `summary` 写得准，路由就准。
...
7. **校准核心摘要 `_core.md`**（见下「核心摘要」）：若本次改动触及**跨场景永远该知道的核心事实/偏好**，更新 `_core.md` 让它反映当前精华；只是某类细节则**不必动核心**（细节留在分类页，按需读）。
8. **自检**：跑 `python3 <substrate-doctor skill 目录>/doctor.py <实例根>`，把所有 `[ERROR]` 修到 0；留意 `_core.md` 超量的 WARN。
9. **提交**：`git add -A && git commit -m "memory: <说明>" && git push`。
10. **汇报**：列出本次创建/更新的文件；若有 Local-only / Forbidden / contested 判定，明确告诉 owner。

## 核心摘要 `_core.md`（常驻小抄的「always 进」部分）

常驻上下文（`substrate-runtime-context`）**每条消息**只把 `memory/about-owner/_core.md` **整段**灌进 agent，分类记忆页只进「记忆目录」、正文按需读。所以 `_core.md` = **你蒸馏出的、跨场景永远该知道的核心**。

- **是什么**：一小段 bullet——主人是谁 + 最关键的几条偏好/约定（如默认语言、关键工作习惯）。**`≤ 3000 字符`**（doctor 超量告警）。下划线前缀 = 结构页，doctor 豁免常规 frontmatter/孤儿检查。
- **谁维护 = 你（agent）蒸馏**：这是「总结」而非「人工挑哪页重要」。记忆增删时（流程第 7 步）重读相关页，把精华**改写**进 `_core.md`，保持精简。
- **超预算怎么办**：把细节**下沉**到对应分类页（并在该页 `summary` 留索引），`_core.md` 只留最关键的。
- **首次生成**：实例已有分类记忆但还没 `_core.md`（doctor 会 ADVICE）→ 通读现有 about-owner 页，蒸馏出第一版 `_core.md`。
- **它不进「记忆目录」、也不需要 ≥2 互链**（结构页）。
