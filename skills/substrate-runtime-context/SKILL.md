...
> 「调用某 skill」= 读它的 `SKILL.md` 并执行。本 skill 自带脚本 `render-context.py`（零依赖，python3 标准库）。
...
```sh
python3 <本 skill 目录>/render-context.py <实例根>    # 打印小抄到 stdout
```

## 怎么接进 runtime（一次性，之后全自动）

注入点与接法**因 runtime 而异**，见对应 adapter（如 `adapters/hermes/README.md`）。两条路：

- **A — runtime 能跑 shell（默认，自助）**：把「`git pull` → `render-context.py` → 注入」接进它的 session-start hook。能跑 shell 的 agent 读本节即可自己接。
- **B — runtime 是纯聊天网关（不能跑 shell）**：用一个后台定时任务（launchd/cron/gateway）刷新小抄文件，再让 runtime 启动时加载该文件。需一个**有 shell 的人/agent 接一次**；之后运行时零 shell。
