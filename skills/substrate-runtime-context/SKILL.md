...
## 关于主人（核心）         ← memory/about-owner/_core.md 整段（agent 蒸馏的核心摘要）
## 关于主人（记忆目录…）    ← 其余分类记忆页的 summary 索引（正文不进、按需用 substrate-memory 读）
## 库里有什么（各区速览）   ← 每个 zone README 顶部的 Agent Packet
## 何时用哪个 skill（路由表）← 从已装 substrate-* skill 的 description 派生
## 房规                     ← house-rules.md（先提议后写、写库即刷、按需细读等常驻规矩）
```

- **定量且不随记忆增长**：只带 `_core.md`（≤3000 字符）+ 各页一行目录；分类记忆全文按需才读。记忆攒到多大，小抄都恒定小。超 `SUBSTRATE_CONTEXT_MAX_CHARS`（默认 12000）只在 stderr 告警，不截断、不失败。
- **核心由 agent 蒸馏**：`substrate-memory` 维护 `_core.md`（总结，不是人工挑页）。
